...
namespace Day11 {
    ...
    static MonkeyLand read_monkeys2(std::string_view sv) {
        MonkeyLand ml;
        ...
        for (auto& chunk: chunks) {
            ...
            monkey.if_false_id = false_monkey_id;
            monkeys.emplace_back(monkey);
            divisor_product *= monkey.divisible_by;
            ml.divisor_product = divisor_product;
        }
        ml.monkeys = monkeys;
        return ml;
    }
    ...
    static void part2(std::string_view sv) {
        MonkeyLand monkey_land = read_monkeys2(sv);
        std::vector<Monkey> monkeys = monkey_land.monkeys;
        std::vector<long> monkey_inspections(monkeys.size());
        ...
        for (int i = 0; i < 10000; i++) {
            for (auto& monkey : monkeys) {
                for (int j = 0; j < monkey.items.size(); j++) {
                    ...
                    switch (monkey.op) {
                        case Operation::Multiplication: {
                            if (monkey.op_value == "old") {
                                ...
                                worry_level = std::stol(monkey.op_value) * item;
                            }
                            break;
                        }
                        case Operation::Addition: {
                    ...
                    auto tmp_level = worry_level;
                    if (worry_level >= monkey_land.divisor_product) {
                        tmp_level = worry_level - ((worry_level / monkey_land.divisor_product) * monkey_land.divisor_product);
                    }
                    if (worry_level % (long)monkey.divisible_by == 0) {
                        monkeys.at(monkey.if_true_id).items.emplace_back(tmp_level);
                    } else {
                        monkeys.at(monkey.if_false_id).items.emplace_back(tmp_level);
                    }
                    monkey_inspections[monkey.id]++;
                }
                monkey.items.clear();
